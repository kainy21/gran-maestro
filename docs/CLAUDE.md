...
```
/mst:config debug.enabled true       # 디버그 모드 활성화
/mst:config debug.log_level debug    # 로그 레벨 변경 (info | debug | trace)
/mst:config debug.log_prompts true   # CLI에 전달되는 프롬프트 내용 로깅
```
