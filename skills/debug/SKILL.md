...
`investigators`는 config의 `participants.opinion_providers`를 읽어 생성합니다.
- `count > 1`일 때는 `codex`, `codex-2`, `codex-3` 형태로 key를 생성합니다.
- `claude` 참여자는 `investigators`에 포함하지 않습니다 (Claude는 자체 조사를 별도 수행).
- 합계 검증: investigators 1~6명 (claude 제외), 위반 시 에러 후 중단.
...
> 모든 호출은 `run_in_background: true`로 실행합니다.

- `provider: "codex"`: `/mst:codex --prompt-file prompts/{investigatorKey}-prompt.md --output finding-{investigatorKey}.md`
- `provider: "gemini"`: `/mst:gemini --prompt-file prompts/{investigatorKey}-prompt.md --files {focus_pattern} > finding-{investigatorKey}.md`
...
- investigators의 동적 배정은 ideation과 동일한 규칙을 따릅니다 (participants.opinion_providers 기반).
- Claude 참여자는 investigators에 포함되지 않고 자체 조사로 분리됩니다.
